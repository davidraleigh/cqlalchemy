...
class _QueryTuple:
    ...
    @staticmethod
    def _build_query(query_tuple: _QueryTuple, filter_query: dict):
        if isinstance(query_tuple.left, _QueryBase):
            ...
            _QueryTuple._build_query(query_tuple.right, filter_query)
        elif isinstance(query_tuple.right, _FilterTuple):
            filter_query["args"].append(query_tuple.right.build_query())
            _QueryTuple._build_query(query_tuple.left, filter_query)
        else:
            _QueryTuple._build_query(query_tuple.left, filter_query)
            _QueryTuple._build_query(query_tuple.right, filter_query)
        return filter_query
    ...
    def build_query(self):
        ...
        filter_query = _QueryTuple._build_query(self, filter_query)
        if self.op == "|":
            filter_query["op"] = "or"
        return filter_query
